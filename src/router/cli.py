...
from router.config.loader import ConfigError, load_node_config
...
def _print_neighbors(node: Node) -> None:
    for n in node.list_neighbors():
        ...
        print(f"  {n.node_id:>3}  {n.host}:{n.port}  costo={n.cost}  estado={estado}  rtt={rtt}")
...
def _print_routes(node: Node) -> None:
    ...
    for dest, entry in sorted(routes.items()):
        print(f"  {dest:>3}  next_hop={entry.next_hop:>3}  costo={entry.cost}")


def run_repl(node: Node) -> None:
    print(f"Nodo {node.node_id} iniciado ({node.config.mode}) en {node.config.host}:{node.config.port}")
